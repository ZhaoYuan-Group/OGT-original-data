...
try:
    # 加载第一个数据文件
    data1 = np.loadtxt('md/new/rmsd-716.dat', comments=['#', '%'])
    time_ns1 = data1[:, 0]/50  # 帧数转ns
    rmsd1 = data1[:, 1]         # RMSD值
    
    # 加载第二个数据文件
    data2 = np.loadtxt('md/new1/rmsd-716.dat', comments=['#', '%'])  # 假设第二个文件名为rmsd-531.dat
...
# 11. 输出
# ======================
output_file = 'rmsd_comparison1.png'
plt.savefig(
    output_file,
    dpi=300,
    bbox_inches='tight',
